...
class ErrorHandlerManager: ErrorHandlerProtocol {
    func handle(_ error: Error) -> Error {
        if let error = error as? NetworkError {
            ...
            switch authError {
            case .notAuthorized:
                return AppError.badCredentials("wrongCredentialsMessage")
            case .errorData(let data):
                return parse(data: data)
            case .noInternet:
    ...
    private func parse(data: Data?, statusCode: Int? = nil) -> AppError {
        ...
        if let errors, error.field != nil {
            if errors.count > 1 {
                let fields = errors.map { $0.field! }
                let messages = errors.map { $0.message }
                return AppError.inputsError(fields, messages)
            } else {
                return AppError.inputError(error.field!, error.message)
            }
        }
